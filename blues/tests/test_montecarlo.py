...
class MonteCarloSimulationTester(unittest.TestCase):
    ...
    def setUp(self):
        ...
        prmtop = testsystems.get_data_filename("data/alanine-dipeptide-explicit/alanine-dipeptide.prmtop")
        inpcrd = testsystems.get_data_filename("data/alanine-dipeptide-explicit/alanine-dipeptide.crd")
        ...
        system_cfg = { 'nonbondedMethod': app.NoCutoff,
        ...
        mc_rep_cfg = { 'stream': { 'title': 'mc',
                                ...
                                'remainingTime': True,
                                'currentIter' : True} }
        mc_reporters = ReporterConfig('ala-dipep-vac', mc_rep_cfg).makeReporters()
        ...
        cfg = { 'nprop' : 1,
                ...
                'nstepsMD': 1,
                'mc_per_iter': 2}
        self.simulations = SimulationFactory(self.systems, self.engine, cfg,
                                              md_reporters=mc_reporters)


    def test_montecarlo_simulationRunPure(self):
        montecarlo = MonteCarloSimulation(self.simulations)
        initial_positions = montecarlo._ncmc_sim.context.getState(getPositions=True).getPositions(asNumpy=True)
        ...
        def nacceptRejectMC(self, temperature=300, **opt):
            ...
            work_mc = (md_state1['potential_energy'] - md_state0['potential_energy']) * (-1.0/montecarlo._ncmc_sim.context._integrator.kT)
            randnum =  math.log(np.random.random())

            if work_mc > randnum:
                montecarlo.accept += 1
                print('MC MOVE ACCEPTED: work_mc {} > randnum {}'.format(work_mc, randnum) )
                montecarlo._md_sim.context.setPositions(md_state1['positions'])
            else:
                montecarlo.reject += 1
                print('MC MOVE REJECTED: work_mc {} < {}'.format(work_mc, randnum) )
        ...
        montecarlo.acceptRejectMC = types.MethodType(nacceptRejectMC, montecarlo)
        montecarlo.run(2)
        #get log acceptance
