...
class SimulationFactoryTester(unittest.TestCase):
    ...
    def test_generateIntegrator(self):
        ...
        cfg = { 'temperature' : 500*unit.kelvin,
    ...
    def test_generateNCMCIntegrator(self):
        ...
        #Check we made the right integrator
        self.assertIsInstance(ncmc_integrator, AlchemicalExternalLangevinIntegrator)
        #Check that the integrator has taken our Parameters
        self.assertAlmostEqual(ncmc_integrator.getTemperature()._value, cfg['temperature']._value)
    ...
    def test_generateSimFromStruct(self):
        ...
        simulation = SimulationFactory.generateSimFromStruct(self.structure, self.system, integrator)
        ...
        struct_box = np.array(self.structure.box_vectors.value_in_unit(unit.nanometers))
        struct_pos = np.array(self.structure.positions.value_in_unit(unit.nanometers))
