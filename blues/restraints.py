...
class BoreschBLUES(Boresch):
    ...
    def restrain_state(self, thermodynamic_state, pose_num=0, force_group=0, 
                       K_com=None, com_ref=None, com_atom_index=None)->openmm.System:
        ...
        for name, value in self._parameters.items():
            if isinstance(value, unit.Quantity):
                logger.info(f'name: {name} and value: {value}')
        ...
        # Add COM restraint (pose-specific version)
        if com_ref is not None and com_atom_index is not None:
            com_restraint = openmm.CustomExternalForce(
                f"restraint_pose_{pose_num} * lambda_restraints * 0.5 * k_com * "
                f"((x - COMx_{pose_num})^2 + (y - COMy_{pose_num})^2 + (z - COMz_{pose_num})^2)"
            )

            com_restraint.addGlobalParameter("k_com", K_com)
            com_restraint.addGlobalParameter(f"COMx_{pose_num}", com_ref[0])
            com_restraint.addGlobalParameter(f"COMy_{pose_num}", com_ref[1])
            com_restraint.addGlobalParameter(f"COMz_{pose_num}", com_ref[2])
            com_restraint.addGlobalParameter("lambda_restraints", 1.0)
            com_restraint.addGlobalParameter(f"restraint_pose_{pose_num}", 0)

            com_restraint.addParticle(com_atom_index, [])
            com_restraint.setForceGroup(force_group + 1)

            system.addForce(com_restraint)
            logger.info(f'[COM-Restraint]: {com_restraint.getEnergyFunction()}')
        
        thermodynamic_state.system = system
        new_sys = thermodynamic_state.get_system(remove_thermostat=True)

        logger.info("Finished restrain_state for COM")
...
class RMSDBlues(RMSD):
    ...
    def restrain_state(self, thermodynamic_state, pose_num=0, force_group=0):
        ...
        logger.info("Finished restrain_state for RMSD")
        logger.info(f"Energy function: {energy_expression}")
        return new_sys
...
def add_rmsd_restraints(sys, struct, pos, ligand_atoms, pose_num=0, force_group=0,
    ...
    logger.info(f"[RMSD-Restraints]Receptor atoms chosen: {rmsd_restraints.restrained_receptor_atoms}")
    logger.info(f"[RMSD-Restraints] Ligand atoms: chosen: {rmsd_restraints.restrained_ligand_atoms}")
    #del thermo
    #del sampler
    #del topography
...
def add_boresch_restraints(sys, struct, pos, ligand_atoms, pose_num=0, force_group=0,
                 restrained_receptor_atoms=None, restrained_ligand_atoms=None,
                 K_r=10, K_angle=10, K_com=None, com_restraint_center=None, 
                 com_atom_index=None, **kwargs):
    """
    Add Boresch and optional COM restraint for a specific pose.
    """

    # Ensure K_r and K_angle are unitless
    if isinstance(K_r, unit.Quantity) or isinstance(K_angle, unit.Quantity):
        raise ValueError("K_r and K_angle must be unitless floats.")

    # Get topology and inject new positions into structure
    topology = struct.topology
    ...
    # Construct restraint object
    boresch = BoreschBLUES(
        restrained_receptor_atoms=restrained_receptor_atoms,
        restrained_ligand_atoms=restrained_ligand_atoms,
        K_r=K_r * unit.kilocalories_per_mole / unit.angstroms**2,
        K_thetaA=K_angle * unit.kilocalories_per_mole / unit.radians**2,
        K_thetaB=K_angle * unit.kilocalories_per_mole / unit.radians**2,
        K_phiA=K_angle * unit.kilocalories_per_mole / unit.radians**2,
        K_phiB=K_angle * unit.kilocalories_per_mole / unit.radians**2,
        K_phiC=K_angle * unit.kilocalories_per_mole / unit.radians**2,
    )

    # Determine missing distances/angles from geometry
    boresch.determine_missing_parameters(thermo, sampler, topography)

    logger.info(f"Receptor atoms: {boresch.restrained_receptor_atoms}")
    logger.info(f"Ligand atoms: {boresch.restrained_ligand_atoms}")



    # Determine COM atom if K_com is specified
    if K_com is not None:
        # Ensure K_com has correct units
        if isinstance(K_com, unit.Quantity):
            K_com = K_com.value_in_unit(unit.kilojoules_per_mole / unit.nanometer**2)
        K_com = K_com * unit.kilojoules_per_mole / unit.nanometer**2

        ligand_pos = np.array(pos)[ligand_atoms]
        ligand_com = ligand_pos.mean(axis=0)
        dists = np.linalg.norm(ligand_pos - ligand_com, axis=1)
        closest_idx = np.argmin(dists)
        com_atom_index = ligand_atoms[closest_idx]
        com_restraint_center = ligand_com
    
    logger.info(f"K_com is: {K_com}")
    logger.info(f"com_ref is: {com_restraint_center}")
    # Apply the force
    new_sys = boresch.restrain_state(thermo, pose_num=pose_num, force_group=force_group,
                                     K_com=K_com, com_ref=com_restraint_center, com_atom_index=com_atom_index)


    logger.info("Custom forces in system:")
    for i, f in enumerate(new_sys.getForces()):
        if isinstance(f, openmm.CustomCompoundBondForce):
            logger.info(f"Force {i}: {f.getName()} with {f.getNumBonds()} bonds")
