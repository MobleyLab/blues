...
if 1:
    ...
    #    def __init__(self, residueList, temperature):
        def __init__(self, **kwds):
            ...
            super().__init__(**kwds)
            pass
#    fie = NetCDFStorage('output.h5')
    dboard = tclass(temperature=300*unit.kelvin, residueList=[141, 142, 143], write_ncmc_interval=1)
...
#    dboard.add_dart((np.array([2.59, 3.34, 2.63]))*unit.nanometers)
    dboard.add_dart((np.array([0.6224805414676666, 1.2787210543950398, 0.5781026184558868]))*unit.nanometers)
    dboard.add_dart((np.array([0.6031394799550374, 0.6303996245066324, 1.3638996283213296]))*unit.nanometers)
    ...
    numIter = 10
#    ncmove = [[dboard.rotationalMove, [1]]]
    ncmove = [[dboard.justdartmove, [2]]]

    dboard.get_particle_masses(testsystem.system)
    numIter = 100
    nstepsNC = 100
    nstepsMD = 1000
    dboard.testintegrator(md_simulation, nc_context, nc_integrator, dummy_simulation, movekey=ncmove, niter=numIter, nstepsNC=nstepsNC, nstepsMD=nstepsMD )
#    dboard.testintegrator(md_simulation, nc_context, nc_integrator, dummy_simulation )
