...
def rigidDart(apos, bpos, rot, centroid_difference, atom_indices=None):
    '''
    ...
    atom_indices
    '''
    if type(atom_indices) == type(None):
...
class Model_MolDart(Model):
    """
    Class for performing smart darting moves during an NCMC simulation.
    """
    def __init__(self, structure, pdb_files, xyz_file, fit_atoms, dart_size, symmetric_atoms=None, resname='LIG', )
        super(Model_MolDart, self).__init__(structure, resname=resname)
        self.dartboard = []
        self.dart_size = []
    ...
    def moldRedart(self, binding_mode_pos, binding_mode_index, nc_pos, symm_pos, bond_compare=True, atom_indices=None):
        """
        Helper function to choose a random pose and determine the vector
        that would translate the current particles to that dart center
        Arguments
...
#
        ...
        if 1:
            ...
            #get appropriate comparision zmat
            zmat_compare = self.internal_zmat[binding_mode_index]
            change_list = ['angle', 'dihedral', 'bond']
...
#            nvec2_dart = vec2_dart / np.linalg.norm(vec2_dart) * self.internal_zmat[rand_index].frame['bond'][self.buildlist[2,0]]/10.
    ...
    def mol_dart(self, nc_context):
        atom_indices = self.atom_indices
        context = nc_context
        stateinfo = context.getState(True, True, False, True, True, False)
        oldEnergy = stateinfo.getPotentialEnergy()
        oldDartPos = stateinfo.getPositions(asNumpy=True)
        selected_pose, diff_list, symm_list = self.poseDart(nc_context)
    ...
    def poseMove(self, context=None, atom_indices=None):
        if atom_indices == None:
            atom_indices = self.atom_indices
        if context == None:
            context = self.nc_context
