...
prot_index = np.array(feat.select("(resid == 125) or (resid == 132) or (resid == 93) or (resid == 102)"))
#added_residues = np.array(feat.select("(resid >= 132) and (resid <= 146) and (name == CA)"))
#lig_coord = np.array(feat.select("(resid 141 to 143)"))
lig_coord = np.array(feat.select("(resid == 142)"))
